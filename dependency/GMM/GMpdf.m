% [p,px_m,pxm,pm_x] = GMpdf(X,mu,S[,pm,o]) Gaussian mixture pdf
%
...
function [p,px_m,pxm,pm_x] = GMpdf(X,mu,S,pm,o)
...
if exist('o','var') & ~isempty(o)
  ...
  [p,px_m,pxm,pm_x] = GMpdf(X(:,o.M),mu,S,pm,[]);
  return;
end
...
switch GMtype(mu,S)
 ...
 case 'D'
  ...
  for m = 1:M
    diffs = bsxfun(@minus,X,mu(m,:));
    ptemp(:,m) = exp(-0.5*sum(diffs.^2/diag(sparse(S(m,:))),2))./(normal*s(m));
  end
end

p = ptemp*pm;  
if nargout>=2 px_m = ptemp +eps ; end;  % add eps in pxm to avoid divide by zero down the line
if nargout>=3 pxm = px_m*diag(sparse(pm)); end; 
if nargout>=4 
  if any(p<eps)
    pm_x = full(diag(sparse((p+eps).^(-1)))*pxm);  % pm_x is not normalized here 
  else
    pm_x = full(diag(sparse(p.^(-1)))*pxm);    
  end;
   pm_x = bsxfun(@rdivide,pm_x,sum(pm_x,2)); % normalize pm_x ; non of the sum(pm_x,2) == 0
end
